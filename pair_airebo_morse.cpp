...
------------------------------------------------------------------------- */
/* ----------------------------------------------------------------------
   AIREBO-M Modified AIREBO using Morse potentials. 
...
PairAIREBO_MORSE::PairAIREBO_MORSE(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  one_coeff = 1;
  ghostneigh = 1;
...
PairAIREBO_MORSE::~PairAIREBO_MORSE()
{
...
void PairAIREBO_MORSE::compute(int eflag, int vflag)
{
  if (eflag || vflag) ev_setup(eflag,vflag);
  else evflag = vflag_fdotr = vflag_atom = 0;

  REBO_neigh();
  if (reboflag) FREBO(eflag,vflag);
...
void PairAIREBO_MORSE::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairAIREBO_MORSE::coeff(int narg, char **arg)
{
...
void PairAIREBO_MORSE::init_style()
{
...
double PairAIREBO_MORSE::init_one(int i, int j)
{
  ...
  //TOC's remapping of the lj1-4 precomputed parameters to values
  //needed for the morse potential 
...
void PairAIREBO_MORSE::REBO_neigh()
{
...
void PairAIREBO_MORSE::FREBO(int eflag, int vflag)
{
...
void PairAIREBO_MORSE::FLJ(int eflag, int vflag)
{
  ...
  double fi[3],fj[3],fk[3],fm[3];
    //TOC variable allocation for MORSE
  double exr;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      //TOC's Morse potential calculation.
      //lj1-4 have been modified to be Morse relvant quantities
      exr = exp(-rij*lj4[itype][jtype]);
...
void PairAIREBO_MORSE::TORSION(int eflag, int vflag)
{
  int i,j,k,l,ii,inum;
  tagint itag,jtag;
  double evdwl,fpair,xtmp,ytmp,ztmp;
...
double PairAIREBO_MORSE::bondorder(int i, int j, double rij[3],
...
double PairAIREBO_MORSE::bondorderLJ(int i, int j, double rij[3], double rijmag,
...
double PairAIREBO_MORSE::gSpline(double costh, double Nij, int typei,
...
double PairAIREBO_MORSE::PijSpline(double NijC, double NijH, int typei, int typej,
                             double dN2[2])
{
  int x,y,i,done;
...
double PairAIREBO_MORSE::piRCSpline(double Nij, double Nji, double Nijconj,
...
double PairAIREBO_MORSE::TijSpline(double Nij, double Nji,
...
void PairAIREBO_MORSE::read_file(char *filename)
{
  int i,j,k,l,limit;
  char s[MAXLINE];
  ...
  //TOC's additional parameters for Morse potential.
  ...
  if (me == 0) {
    ...
    //TOC's lines for reading in MORSE parameters from CH.airebo file
    fgets(s,MAXLINE,fp);
  ...
  if (me == 0) {
    ...
    // TOC's MORSE parameter assignments
  ...
  //TOC's MORSE parameter broadcast
...
double PairAIREBO_MORSE::Sp5th(double x, double coeffs[6], double *df)
{
  double f, d;
  const double x2 = x*x;
...
double PairAIREBO_MORSE::Spbicubic(double x, double y,
...
double PairAIREBO_MORSE::Sptricubic(double x, double y, double z,
...
void PairAIREBO_MORSE::spline_init()
{
  int i,j,k;
...
double PairAIREBO_MORSE::memory_usage()
{
  double bytes = 0.0;
